...
2) Manual
	2.1) Loading XML Files
	2.2) Displaying one TestRun in the class Table
	2.3) Display the change of a classes failure percentage over all testruns
	2.4) Compare two testruns of one class
	2.5) Dependencies between failed classes
...
2.4)	Compare two testruns of one class
		...
		To compare different testruns right-click on the corresponding node on the line chart and add it to one of the comparison
		slots. When both are filled you can use the Compare - Button to see directly which Unit Tests failed or passed now and before.
		Note that you cannot compare two different classes.
		
2.5)	Dependencies between failed classes

		To see the dependencies between two failed classes click on the Analyzer Tab on the main window.
		There you will see the Strong Rule. In each row there are classes that are probably dependend with a given confidence.
		Use the sliders to set the minimum confidence aswell as the minimum distance of two classes.
