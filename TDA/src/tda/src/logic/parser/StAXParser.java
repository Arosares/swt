				...
				case XMLStreamConstants.START_ELEMENT:
					if ("UnitTest".equals(reader.getLocalName())) {
						if (reader.getAttributeLocalName(0).equals("id")
							...
							throw new WrongXMLAttributeException(
									"Expected another attribute in tag " + reader.getLocalName());
						}
					...
					if ("Execution".equals(reader.getLocalName()) && reader.getAttributeCount() == 1) {
						if (reader.getAttributeLocalName(0).equals("id")) {
					...
					if ("Counters".equals(reader.getLocalName())) {
						if (reader.getAttributeLocalName(0).equals("aborted")
							...
							Counters counter = new Counters(sumAborted, sumCompleted, sumDisconnected, sumError,
									sumExecuted, sumFailed, sumInProgress, sumInconclusive, sumNotExecuted,
									sumNotRunnable, sumPassed, sumPassedButRunAborted, sumPending, sumTimeOut, sumTotal,
									sumWarning);
